...
def openChannel(channel_number, channel_type, bps=None):
    ...
    Args:
        ...
        channel_type (linlib.ChannelType): Whether the LIN interface will be a
            master or slave.
...
class Channel(object):
    ...
    def getFirmwareVersion(self):
        ...
        ``app_version`` that are `VersionNumber` namedtuples. If only one of
        these is needed, the return value can be unpacked as such::

            boot_ver, app_ver = channel.getFirmwareVersion()

        Note:
            For newer interfaces use `linlib.getChannelData` with
            `linlib.ChannelData.CARD_FIRMWARE_REV` instead.
    ...
    def setupIllegalMessage(self, msgid, disturb_flags, delay):
        ...
        Args:
            ...
            disturb_flags (linlib.MessageDisturb): One or more of the
                `linlib.MessageDisturb` flags.
    ...
    def updateMessage(self, frame):
        ...
        Args:
            ...
            frame (Frame): The information to be updated. Only the ~id~,
                ~data~, and ~dlc~ attributes are used. Note that the frame can,
    ...
    def writeWakeup(self, count=0, interval=1):
        ...
        spaced with `interval` bittimes. The LIN interface will interrupt the
